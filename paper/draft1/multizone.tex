...
Galactic regions, making them a more realistic option than classical one-zone
...
parameterizes the Galaxy disc as a series of concentric rings.
...
nucleosynthetic products in a ring other than the one they were born in, each
ring is otherwise described by a conventional one-zone GCE model.
Allowing stars to enrich a distribution of radii was a novel addition to these
models, and~\citet{Johnson2021} demonstrated that this has a significant impact
on enrichment rates from delayed sources such as SNe Ia.
\par
...
Although our model neglects the effects of blurring by assuming smooth
migration with a~$\sqrt{\text{age}}$ dependence (see discussion above), all of
these effects are present in~\hsim, and consequently realistic churning should
simply be inherited by the stellar populations in our model.
\par
We assume the SFH of the ``inside-out'' model from~\citet{Johnson2021}.
The time-dependence at a known~\rgal~is given by
\begin{equation}
...
GCE models often adopt a single power-law relating the surface density of gas
$\Sigma_\text{gas}$ to the surface density of star formation
$\dot{\Sigma}_\star$ based on the findings of~\citet{Kennicutt1998}.
...
three-component power-law as their $\dot{\Sigma}_\star - \Sigma_\text{gas}$
relation with index given by:
\begin{equation}
N =
\begin{cases}
...
\citep{Leroy2008, Leroy2013} with a~$t^{1/2}$ time-dependence based on the
findings of~\citet{Tacconi2018} studying the redshift evolution of the
$\Sigma_\text{gas} - \dot{\Sigma}_\star$ relation.
\par
...
their~\S~3.1).
Nonetheless, these parameters are mutually degenerate since they act as source
and sink terms in computing enrichment rates.
